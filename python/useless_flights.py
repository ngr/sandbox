...
def get_smart_price(source, destination, best = 1000) -> List:
    ...
    for src, value in by_destination[destination]:
        ...
        smart_price = get_smart_price(src, destination, best_price)

        # if not smart_price or smart_price == 1000:
        #     return None
        if smart_price and smart_price <= best_price:
            best_price = smart_price
            break
        else:
            return best_price
...
def useless_flight(schedule: List) -> List:
    ...
    for i, (source, destination, cost) in enumerate(schedule):
        ...
        if get_best_price(source, destination) < cost:
            print(f"already in best price {source}, {destination}, {best_price}")
            result.append(i)
            continue
        cost = get_smart_price(source, destination)
